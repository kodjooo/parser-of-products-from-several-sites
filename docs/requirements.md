...
— Долгие блокировки пережидаем автоматически: счётчики подряд идущих неудачных загрузок категорий и карточек увеличиваются на каждом фейле; теперь учитываются отдельные таймауты Playwright (повторы), поэтому агент не ждёт, пока сменится номер страницы — после `FAIL_COOLDOWN_THRESHOLD` таймаутов подряд он пишет предупреждение (причина — временные лимиты сайта/прокси), выдерживает паузу `FAIL_COOLDOWN_SECONDS`, после чего обнуляет счётчик и продолжает обход. Параметры задаются через `.env`, что позволяет отключать механизм (threshold=0) или менять длительность паузы под конкретный магазин.
