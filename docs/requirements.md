...
2.4. (Опционально) Фиксированный лимит страниц по каждой категории задаётся в category_pages (ключ = URL категории, значение = число страниц).
...
— Долгие блокировки пережидаем автоматически: счётчики подряд идущих неудачных загрузок категорий и карточек увеличиваются на каждом фейле; теперь учитываются отдельные таймауты Playwright (повторы), поэтому агент не ждёт, пока сменится номер страницы — после `FAIL_COOLDOWN_THRESHOLD` таймаутов подряд он пишет предупреждение (причина — временные лимиты сайта/прокси), принудительно сбрасывает буфер и даёт сигнал внешнему watchdog-скрипту (`scripts/cooldown_watchdog.py`), который выполняет `docker compose down && up -d --build parser`. Если watchdog не используется, можно включить старую паузу, задав `FAIL_COOLDOWN_SECONDS` > 0. Параметры задаются через `.env`, что позволяет отключать механизм (threshold=0) или менять длительность паузы под конкретный магазин.
