...
- В `SiteCrawler` встроен блок «cooldown»: счётчики подряд идущих ошибок загрузки страниц категорий увеличиваются на каждом исключении Playwright/HTTP-движка. Как только серия достигает порога `FAIL_COOLDOWN_THRESHOLD`, краулер фиксирует предупреждение, принудительно сбрасывает буфер записей в Google Sheets и уходит на паузу `FAIL_COOLDOWN_SECONDS`, чтобы переждать временный бан. После паузы счётчик обнуляется и обход продолжается с того же места.
