...
- Compose-сервис `parser` запускается с политикой `restart: always`, поэтому Docker сам поднимает контейнер заново после завершения CLI. Это гарантирует автоматическое «воскрешение» пула прокси: при каждом рестарте в памяти формируется новый `ProxyPool`, и список заблокированных адресов сбрасывается вместе с остальными счётчиками.
