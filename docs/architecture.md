...
- Если категория внезапно возвращает 0 ссылок (хотя селектор указан), `SiteCrawler` запускает расписание повторов: 1, 10, 20 и 60 минут ожидания с обязательной сменой прокси перед каждой попыткой. После каждой паузы страница загружается заново, и только после исчерпания всех повторов обход категории останавливается. Это позволяет дождаться снятия блокировки и не терять данные.
