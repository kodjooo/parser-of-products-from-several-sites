...
- В `SiteCrawler` встроен блок «cooldown»: счётчики подряд идущих ошибок загрузки страниц категорий увеличиваются на каждом исключении Playwright/HTTP-движка. Дополнительно трекаются сами таймауты движка: как только Playwright пять раз подряд (или сколько задано `FAIL_COOLDOWN_THRESHOLD`) не смог отрендерить страницу, краулер фиксирует предупреждение, принудительно сбрасывает буфер записей в Google Sheets, выставляет внутренний флаг cooldown и прекращает дальнейший обход категорий/страниц до рестарта. Вместо `time.sleep` по умолчанию запускается watchdog (`scripts/cooldown_watchdog.py`), который, заметив предупреждение в логе, выполняет `docker compose down` + `docker compose up -d --build parser`. Watchdog умеет переживать усечение и ротацию `parser.log`: при смене inode или уменьшении размера он переоткрывает файл или смещается в начало. Для запуска внутри той же `docker compose`-связки добавлен режим `--restart-mode stack` — он перезапускает всю связку, обеспечивая полный сброс окружения. Watchdog поддерживает таймауты на команды (`--command-timeout`) и повторы рестарта (`--retry-attempts`, `--retry-delay-seconds`), чтобы не зависать на сетевых сбоях. Параметр `FAIL_COOLDOWN_SECONDS` оставлен для обратной совместимости: если задать его больше нуля, краулер сделает паузу и продолжит обход с того же места без перезапуска контейнера.
...
- Конфигурация описывается в `RuntimeConfig.behavior` и настраивается через блок `BEHAVIOR_*` в `.env`/общей YAML-конфигурации: вероятности скролла, диапазоны глубин, количество движений мыши, селекторы hover и лимиты дополнительных переходов (`max_additional_chain`, `extra_products_limit`, `visit_root_probability` и т. п.). Лог скроллов пишет числовые проценты, включая возврат наверх как `scroll:0`, чтобы метрики и тесты не ломались на строковых маркерах.
...
- Практический сценарий пришёл из реальных логов (`logs/*/parser.log`): винный магазин позволяет выгружать витрину только «пачками» (5–6 категорий подряд), затем весь трафик на час получает HTTP 403. Режим `watch` по умолчанию перезапускает агент сразу после падения и, без дополнительной логики, моментально упирался в тот же лимит, зацикливая ошибки и не давая записать уже собранный буфер. Теперь SiteCrawler и ContentFetcher ведут собственные счётчики подряд идущих фейлов; при достижении `FAIL_COOLDOWN_THRESHOLD` (по умолчанию 5) они логируют русскоязычное предупреждение с причиной «временные лимиты сайта» и принудительно сбрасывают накопленный буфер в Sheets. Внешний watchdog (`scripts/cooldown_watchdog.py`) по этому предупреждению перезапускает контейнер (`docker compose down/up` или в режиме `--restart-mode service` — `docker compose stop/rm/up parser`), что даёт «чистый» браузер/прокси. Если watchdog не используется, можно включить fallback-паузу, задав `FAIL_COOLDOWN_SECONDS` > 0 — тогда краулер подождёт указанное время и продолжит обход без рестарта.
