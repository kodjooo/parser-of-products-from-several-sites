...
- `app.crawler.engines.BrowserEngine` — помимо быстрых ретраев (теперь они выполняются с задержками 30 и 60 секунд) добавляет длинные повторы: спустя 2 и 4 минуты выполняются дополнительные попытки загрузки страницы через новый прокси/прямой IP. Это повышает шанс пройти временные блокировки страниц с глубокой пагинацией.
