...
- Если страница категории оказалась пустой, агент не завершает обход сразу: сначала выполняются повторные загрузки через другой прокси (1/10/20/60 минут ожидания), а при отсутствии результата переходит к следующей странице. Обход категории прекращается только после трёх подряд пустых страниц выданной пагинации.
- Если URL товара уже встречался в текущем запуске или ранее записан на вкладке, он не отправляется повторно, а ссылка и причина сохраняются в `state/duplicate_products.log`, чтобы можно было проверить, какие карточки отфильтрованы как дубликаты.
