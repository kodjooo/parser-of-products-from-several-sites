...
- `FAIL_COOLDOWN_SECONDS` — сколько секунд ждать перед повтором серии (за это время прокси/сайт снимает временный бан).

После достижения порога краулер пишет предупреждение, принудительно отправляет накопленный буфер в Sheets и делает `time.sleep` на указанное время. Теперь счётчик считается не по страницам, а по отдельным таймаутам Playwright: если подряд произошло N неудачных попыток загрузки (по умолчанию 5), агент сразу уходит в паузу, не дожидаясь, пока сменится номер страницы. Значения можно уменьшить для тестовых стендов или отключить механизм (threshold=0). Пауза не ломает циклы `watch`: после пробуждения агент продолжает обход с того же места.
