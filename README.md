...
## Основные компоненты
- `app/cli.py` — Typer-CLI (`python -m app.main ...`) с одиночным запуском (`run`) и непрерывным режимом (`watch`), который сам перезапускает агент после завершения или ошибки.
...
### Локальные конфиги сайтов
Каталог `config/sites` исключён из Git (см. `.gitignore`), поэтому боевые YAML-файлы держим только локально или в отдельных зашифрованных репозиториях. Чтобы подключить новый сайт:
- скопируйте шаблон `*.yml` в `config/sites/<site>.yml` и заполните селекторы/категории;
- добавьте каталог в volume при запуске контейнера (`-v $(pwd)/config/sites:/app/config/sites`);
- не коммитьте эти файлы в основной репозиторий — для разных окружений можно хранить несколько каталогов и переключать их через `SITE_CONFIG_DIR`.

## Настройка пауз при блокировках
Некоторые сайты (см. логи `logs/parser.log` для winestyle) позволяют загружать только несколько страниц подряд и дальше на час блокируют IP. Чтобы не зациклить `watch`-режим на постоянных ошибках, в `.env` добавлены параметры:
- `FAIL_COOLDOWN_THRESHOLD` — сколько подряд неудачных загрузок категории/карточки считаем тревожной серией;
- `FAIL_COOLDOWN_SECONDS` — сколько секунд ждать перед повтором (по умолчанию 0, используем watchdog-скрипт для перезапуска контейнера; задайте значение >0, если нужна старая пауза).

После достижения порога краулер пишет предупреждение и принудительно отправляет накопленный буфер в Sheets. Теперь счётчик считается не по страницам, а по отдельным таймаутам Playwright: если подряд произошло N неудачных попыток загрузки (по умолчанию 5), агент мгновенно сигнализирует. Ждать целый час больше не нужно — внешний watchdog (`scripts/cooldown_watchdog.py`) видит предупреждение в `parser.log` и выполняет `docker compose down && docker compose up -d --build parser`. Если watchdog ещё не включен, агент просто продолжит обход без паузы; чтобы вернуть прежнее ожидание, задайте `FAIL_COOLDOWN_SECONDS` больше нуля.

### Watchdog для автоматического перезапуска
`scripts/cooldown_watchdog.py` считывает `parser.log` (tail -f) и реагирует на строки `Достигнут предел подряд…`. Пример запуска:
```bash
python scripts/cooldown_watchdog.py \
  --log-file /usr/local/other-scripts/parser-of-products-from-several-sites/logs/parser.log \
  --project-dir /usr/local/other-scripts/parser-of-products-from-several-sites \
  --service parser
```
...
Для запуска целой связки одной командой добавлен сервис `watchdog` в `docker-compose.yml`. Он собирается из образа `Dockerfile.watchdog`, монтирует `docker.sock` и вызывает скрипт в режиме `--restart-mode service`, поэтому перезапускает только контейнер `parser`, не останавливая себя. По умолчанию watchdog отключает BuildKit (`--buildkit off`) и пересборку (`--build off`), чтобы избегать сетевых проблем при рестарте; при необходимости можно вернуть пересборку и настроить `--command-timeout`. Запуск и остановка теперь выглядят так:
...
- `python -m app.main watch` — держит агент онлайн 24/7: после каждого успешного прохода делает паузу (`--success-delay`, по умолчанию 300 секунд), а при ошибке ждёт `--error-delay` (120 секунд) и автоматически перезапускает процесс. Режим поддерживает ограничение числа итераций (`--max-runs`) и работает совместно с `--resume`, поэтому каждый цикл продолжает обход с последней сохранённой страницы.
...
Команда автоматически подхватит `.env` из корня и смонтирует нужные volume (`config/sites`, `state`, `assets/images`, `secrets`, `logs`). Каталоги должны существовать заранее. Сервис записывает историю запусков в `logs/parser.log`, а прокси/IP, получившие два ответа 403 подряд, фиксирует в `logs/bad_proxies.log`, после чего помечает их как испорченные и не использует дальше. Время, через которое такие источники автоматически возвращаются в пул, настраивается переменной `NETWORK_PROXY_REVIVE_AFTER_MINUTES` (по умолчанию 30 минут). При успешной загрузке страницы прокси немедленно снимается с блокировки, поэтому единичные сбои больше не «сжигают» весь пул.
Если страница подвисает, Playwright сначала делает быстрые ретраи с паузами, заданными в `NETWORK_RETRY_BACKOFF_SEC` (по умолчанию 30 и 60 секунд, каждый раз меняя прокси), а затем автоматически выполняет ещё три длинных попытки через 2, 4 и 15 минут. Все ожидания фиксируются в `logs/parser.log`.
