...
После достижения порога краулер пишет предупреждение, принудительно отправляет накопленный буфер в Sheets и делает `time.sleep` на указанное время. По умолчанию это 5 неудач и 3600 секунд отдыха, но значения можно уменьшить для тестовых стендов или отключить механизм (threshold=0). Пауза не ломает циклы `watch`: после пробуждения агент продолжает обход с того же места.
