...
- `python -m app.main run` — одиночный обход (именно эта команда теперь используется в `docker-compose.yml`, потому что перезапусками занимается внешний watchdog).
