...
После достижения порога краулер пишет предупреждение и принудительно отправляет накопленный буфер в Sheets. Теперь счётчик считается не по страницам, а по отдельным таймаутам Playwright: если подряд произошло N неудачных попыток загрузки (по умолчанию 5), агент мгновенно сигнализирует и прекращает дальнейший обход до рестарта. Ждать целый час больше не нужно — внешний watchdog (`scripts/cooldown_watchdog.py`) видит предупреждение в `parser.log` и выполняет `docker compose down && docker compose up -d --build parser`. Watchdog игнорирует строки, старше момента запуска, чтобы не перезапускать контейнер на «старых» сигналах. Если watchdog ещё не включен, агент просто продолжит обход без паузы; чтобы вернуть прежнее ожидание, задайте `FAIL_COOLDOWN_SECONDS` больше нуля.
